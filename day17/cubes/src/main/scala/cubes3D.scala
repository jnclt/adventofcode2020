...
object Cubes3D extends App {
    if (args.length != 2) {
        println("Number of steps and input file required!")
        System.exit(1)
    }

    case class Cell(z: Int, y: Int, x: Int)
    type Space = Set[Cell]
    ...
    def step(size: Cell)(space: Space, offset: Int): Space = {
        val next =
            for (z <- offset to (size.z - 1 - offset);
    ...
    def init_space(lines: Vector[String], steps: Int): (Space, Cell) = {
        val space = 
            for ((line, y) <- lines.zipWithIndex;
                 ...
                 yield Cell(0 + steps, y + steps, x + steps)
    ...
    def pprint(space: Space, size: Cell) = {
        ...
        for (plane <- 0 to size.z - 1 ) {
            for (line <- 0 to size.y - 1) {
                println((0 to size.x - 1).map( cell => if (space.contains(Cell(plane, line, cell))) '#' else '.').mkString)
            }
            println()
        }
    }
